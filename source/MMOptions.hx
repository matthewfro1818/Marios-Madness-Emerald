...
{
	var options:Array<String> = ['Notes', 'Controls', 'Graphics', 'Gameplay', 'Mario Options', 'Delete Data'];
	...
	override function create()
	{
		...
		verText.setFormat(Paths.font("mariones.ttf"), 16, 0xFF00FF19, LEFT);
...
class NotesSubstate extends MusicBeatSubstate
{
	...
	function changeSelection(change:Int = 0)
	{
		...
		for (i in 0...grpNumbers.length)
		{
			...
			item.alpha = 0.4; item.color = 0xFFFFFFFF;
			if ((curSelected * 3) + typeSelected == i) {item.alpha = 1; item.color = 0xFFFFFFFF;}
		}
		for (i in 0...grpNotes.length)
		{
			var item = grpNotes.members[i];
			...
			if (curSelected == i)
			{
				item.alpha = 1; item.color = 0xFFFFFFFF;
				item.scale.set(.9, .9);
				hsvText.setPosition(item.x + hsvTextOffsets[0], item.y - hsvTextOffsets[1]);
			}
		}
		...
		if (usesCheckbox)
		{
			if (controls.ACCEPT && nextAccept <= 0)
			{
				switch (options[curSelected])
				{
					...
					case 'Shaders':
						ClientPrefs.shadersToggle = !ClientPrefs.shadersToggle;
		...
		for (i in 0...checkboxArray.length)
		{
			...
			if (checkbox != null)
			{
				...
				switch (options[checkboxNumber[i]])
				{
					...
					case 'Shaders':
						daValue = ClientPrefs.shadersToggle;
				}
