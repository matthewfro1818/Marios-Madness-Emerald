...
class PlayState extends MusicBeatState
{
	...
	public static var disabledMiddlescroll:Bool = false;

	var noMiddlescrollStages:Array<String> = ['somari', 'piracy'];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'allfinal':
				...
				addCharacterToList('m3', 1);
				addCharacterToList('m4', 1);
				addCharacterToList('m5', 1);
		...
		strumLine = new FlxSprite(ClientPrefs.middleScroll ? -320 : 42, 50).makeGraphic(FlxG.width, 10);
		// if (hasDownScroll)
		...
		{
			timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 20, 400, "", 32);
			timeTxt.setFormat(Paths.font("mario2.ttf"), 16, gbcolor, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}
		...
		luigiLogo = new FlxSprite(400, 500);
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			for (i in 0...opponentStrums.length) {
				...
				if(ClientPrefs.middleScroll) opponentStrums.members[i].visible = true;
			}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? -320 : 42, strumLine.y, i, player);
	...
	var countdown1:Int = 0;
	var countdown2:Int = 0;
	var rng:FlxRandom = new FlxRandom();
	public var removeHealth:Float;
	...
	override public function update(elapsed:Float)
	{
		...
		if(curStage == 'nesbeat'){
			...
			if (countdown1 >= 70 && curStep >= 127 && curStep < 6096){
				if (curStep >= 4815 && curStep < 5584){
					removeHealth = rng.float(0.035,0.050);
					if (health >= 0.350){
						health -= removeHealth;
					}
				}else{
					removeHealth = rng.float(0.025,0.035);
					if (health >= 0.200){
						health -= removeHealth;
					}
				}
				trace("Hp Drained: " + removeHealth);
				countdown1 = 0;
			}
			if (countdown2 >= 200){
				if (curStep >= 4815 && curStep < 5584){
					SONG.speed = rng.float(3.2,3.5);
				}else if (curStep >= 5584){
					SONG.speed = rng.float(3.5,4.2);
				}else if (curStep < 4815){
					SONG.speed = rng.float(2.9,3.5);
				}
				trace("Current Scroll Speed: " + SONG.speed);
				countdown2 = 0;
			}
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				{
					daNote.active = true;
					if(!songIsModcharted) daNote.visible = true;
				}
	...
	public function endSong():Void
	{
		...
		if (disabledMiddlescroll){
			ClientPrefs.middleScroll = true;
			disabledMiddlescroll = false;
		}
	 ...
	 */
		...
		if (curStage == 'virtual')
		{
			comboSpr.color = 0xFF1900F;
			rating.color = 0xFF1900F;
		}
		else
		{
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote){
				if (curStage == 'nesbeat'){
					health += note.missHealth + note.hitHealth;
				}else{
