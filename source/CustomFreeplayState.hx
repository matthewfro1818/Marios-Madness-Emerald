...
class CustomFreeplayState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		for (i in 0...options.length)
		{
			...
			imagePortrait.color = 0xFF00FF19; imagePortrait.alpha = 0.9;
		...
		//FlxTween.color(MainMenuState.instance.estatica, .8, MainMenuState.instance.estatica.color, 0xFF00FF19);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK && !Main.selectingdiff)
		{
	...
	function goToState()
	{
		...
		switch (options[curSelected])
		{
			...
			case 'Legacy':
				canciones = [
					["It's a me (Legacy)", 'its-a-me-old', 		'1'],
					['Golden Land (Legacy)', 'golden-land-old', 	'2'],
					['I Hate You (Legacy)', 'i-hate-you-old', 	'3'],
					['Powerdown (Legacy)', 'powerdown-old', 		'4'],
					['Apparition (Legacy)', 'apparition-old', 	'5'],
					['Forbidden Star (Apparition Legacy Original)', 'forbidden-star', 			'39'],
					['Alone (Legacy)', 'alone-old', 				'6'],
					['Race-traitors (Legacy)', 'racetraitors-old', '7'],
...
//		FlxTween.tween(FlxG.camera, {zoom: 0.75}, 1, {ease: FlxEase.quadOut});
		...
		for (i in 0...CustomFreeplayState.canciones.length)
		{
			...
			char.ID = i;
			if(i != curSelected) char.color = 0xFF00FF19;
			boxgrp.add(char);
		...
		descText.color = 0xFF00FF19;
		boxgrp.color = FlxColor.LIME;
		cartel.color = 0xFF00FF19;
	...
	override function update(elapsed:Float)
	{
		...
		var rightP = controls.UI_RIGHT_P;
			...
			{
				if (CustomFreeplayState.canciones[curSelected][1] == 'last-course' || CustomFreeplayState.canciones[curSelected][0] == 'Last Course' || CustomFreeplayState.canciones[curSelected][2] == '18'){
					Main.selectingdiff = true;
					openSubState(new LastCourseDiff());
				}else{
					goToSong();
				}
			}
		...
		if (controls.BACK && !quieto && !Main.selectingdiff)
		{
			curSelected = 0;
	...
	function goToSong()
	{
		...
		tween = FlxTween.tween(MainMenuState.instance.estatica, {alpha: 0}, 2, {
			...
			onComplete: function(twn:FlxTween)
			{
				...
				FlxTween.color(MainMenuState.instance.estatica, .8, MainMenuState.instance.estatica.color, 0xFFFFFFFF);
		...
		boxgrp.forEach(function(char:FlxSprite)
			{
				if (char.ID == curSelected)
				{
					...
					FlxTween.color(char, 0.2, 0xFF00FF19, FlxColor.LIME, {ease: FlxEase.expoOut});
				}
				else{
					if(char.color != 0xFF00FF19){
					FlxTween.tween(char, {y: 150 + (270 - (char.height / 2))}, 0.2, {ease: FlxEase.expoOut});
					FlxTween.color(char, 0.2, FlxColor.LIME, 0xFF00FF19, {ease: FlxEase.expoOut});
					}
				}
			});
	}
