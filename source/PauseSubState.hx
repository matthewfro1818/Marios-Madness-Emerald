	...
	override function update(elapsed:Float)
	{
		...
		if (accepted && !selected)
		{
			...
			switch (daSelected)
			{
				...
				case 'Botplay':
						FlxG.save.data.luigiMode = !FlxG.save.data.luigiMode;
						PlayState.usedPractice = true;
						botplayText.visible = FlxG.save.data.luigiMode;
						PlayState.cpuControlled = FlxG.save.data.luigiMode;
						trace('Botplay Values: ' + FlxG.save.data.luigiMode + "," + PlayState.cpuControlled);
						if (FlxG.save.data.luigiMode == true && PlayState.cpuControlled == true){
							trace('Botplay Successfully Enabled');
						}else if (FlxG.save.data.luigiMode == true && PlayState.cpuControlled == false
							|| FlxG.save.data.luigiMode == false && PlayState.cpuControlled == true){
							trace('Botplay Failed To Enable');
						}else if (FlxG.save.data.luigiMode == false && PlayState.cpuControlled == false){
							trace('Botplay Successfully Disabled');
						}
				case "Exit":
					hideall();

					if (PlayState.disabledMiddlescroll){
						ClientPrefs.middleScroll = true;
						PlayState.disabledMiddlescroll = false;
					}
